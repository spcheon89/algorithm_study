...
int main()
{
    ...
    for(int i=1; i<=test_case; i++)
    {
        ...
        // stone 위치와 0 사이의 거리가 최소가 되는 point 찾기
        for(int j=0; j<N; j++)
        {
            cin >> stone_position[j];
            if(abs(stone_position[j]) < min_position)
                min_position = abs(stone_position[j]);
        }
        // min_position을 갖는 stone 갯수 count
        for(int j=0; j<N; j++)
        {
            if(min_position == abs(stone_position[j]))
                person_cnt++;
        }
        // output
        cout << "#" << i << " " << min_position << " " << person_cnt << "\n";
    }
